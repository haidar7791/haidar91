...
export default function App() {
  ...
  const addBuilding = (type) => {
    ...
    let cost = 0;
    if(type === "worker_hut" && buildings.length > 0) cost = 1; // crystals for additional huts
    if(type === "worker_hut" && buildings.length === 0) cost = 0; // first hut free
  ...
  const upgradeBuilding = (building) => {
    ...
    let costAmount = 100 * building.level; // مثال: التكلفة تتضاعف حسب المستوى
